...
export class BlcalculatorComponent {
  ...
  // Diminishing Loan
  diminishingloanAmount: number = 5000000;
  ...
  limitInputLength(event: Event): void {
    const input = event.target as HTMLInputElement;
    let value = input.value.slice(0, 9);
    const num = Number(value);
    if (num > 100000000) {
      value = '100000000';
    }
  ...
  limitInputLengthfordiminishing(
    event: Event,
  ...
  calculateEMI(): void {
    const totalInterest =
      (this.loanAmount * this.interestRate * this.tenure) / 100;
  ...
  generatePDF(): void {
    ...
    const addHeader = () => {
      ...
      doc.text(
        businessName?.toUpperCase() || 'LOAN REPORT',
        pageWidth / 2,
        15,
        { align: 'center' }
      );
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          ...
          `${this.interestRate}%`,
          'Interest:',
          `Rs. ${this.totalInterest}`,
        ],
        [
          'Tenure:',
          `${this.tenure} years`,
          'Total Payable:',
          `Rs. ${this.totalPayable}`,
        ],
        [
          'CPP (One Time Charges):',
          `${this.cppCharges}`,
          'PF % On Loan Amount (One Time Charges):',
          ` ${this.pfAmount}`,
        ],
      ],
      theme: 'grid',
  ...
  generatediminishingPDF(): void {
    ...
    autoTable(doc, {
      ...
      body: [
        [
          'Loan Amount:',
          `Rs. ${this.diminishingloanAmount}`,
          'Monthly EMI:',
          `Rs. ${this.diminishingemi}`,
        ],
        [
          'Interest Rate (Diminishing):',
          `${this.diminishinginterestRate}%`,
          'Interest:',
          `Rs. ${this.diminishingtotalInterest}`,
        ],
        [
          'Tenure:',
          `${this.diminishingtenure} years`,
          'Total Payable:',
          `Rs. ${this.diminishingtotalPayable}`,
        ],
        [
          'CPP (One Time Charges):',
          `${this.cppCharges}`,
